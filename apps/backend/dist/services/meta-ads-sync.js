 ...
 */
async function syncCampaigns(organizationId, platformId, accountId, adAccountId, client) {
 ...
 */
async function syncAds(organizationId, platformId, accountId, adAccountId, client) {
 ...
 */
async function syncCostInsights(organizationId, platformId, accountId, adAccountId, client, startDate, endDate) {
 ...
 */
export async function syncMetaAdsData(organizationId, options = {}) {
    try {
        // Get access token
        ...
        for (const adAccountId of accountIds) {
            ...
            if (!account) {
                ...
                const accountData = await client.request(adAccountId, {
            ...
            // Sync campaigns
            const campaignCount = await syncCampaigns(organizationId, platform.id, account.id, adAccountId, client);
            totalCampaigns += campaignCount;
            // Sync ads
            const adCount = await syncAds(organizationId, platform.id, account.id, adAccountId, client);
            totalAds += adCount;
            ...
            // Sync cost insights
            const metricsResult = await syncCostInsights(organizationId, platform.id, account.id, adAccountId, client, startDate, endDate);
...
//# sourceMappingURL=meta-ads-sync.js.map
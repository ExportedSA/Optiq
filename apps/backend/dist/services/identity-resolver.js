 ...
 */
function generateIdentityKey(signals) {
 ...
 */
async function findMatchingIdentities(organizationId, siteId, signals) {
    const conditions = [];
    if (signals.emailHash) {
 ...
 */
async function mergeIdentities(organizationId, siteId, identityIds, signals, mergeReason) {
 ...
 */
async function createIdentity(organizationId, siteId, signals) {
 ...
 */
export async function resolveIdentity(organizationId, siteId, signals) {
    ...
    if (matchingIdentityIds.length === 1) {
        ...
        const existingAliasKeys = new Set(existingAliases.map((a) => `${a.aliasType}:${a.aliasValue}`));
        const newAliases = [];
    ...
    const mergeReason = signals.emailHash
        ...
        : signals.phoneHash
            ...
            : signals.customerId
                ? "customerId"
                : "deviceId";
    const targetIdentityId = await mergeIdentities(organizationId, siteId, matchingIdentityIds, signals, mergeReason);
    const identity = await prisma.identity.findUnique({
 ...
 * Get identity by key
 */
export async function getIdentityByKey(identityKey) {
 ...
 */
export async function getIdentityMergeHistory(identityId) {

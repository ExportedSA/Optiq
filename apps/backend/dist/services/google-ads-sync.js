 ...
 */
async function fetchCampaigns(accessToken, customerId) {
    const query = `
    SELECT
      campaign.id,
      campaign.name,
  ...
  `;
    const response = await fetch(`${GOOGLE_ADS_API_URL}/customers/${customerId}/googleAds:searchStream`, {
        method: "POST",
        headers: {
 ...
 */
async function fetchCostMetrics(accessToken, customerId, startDate, endDate, timezone) {
    const query = `
    SELECT
      segments.date,
      campaign.id,
 ...
 */
async function syncCampaigns(organizationId, platformId, accountId, customerId, accessToken) {
 ...
 */
async function syncCostMetrics(organizationId, platformId, accountId, customerId, accessToken, startDate, endDate, timezone) {
 ...
 */
export async function syncGoogleAdsData(organizationId, options = {}) {
    try {
        // Get access token
        ...
        for (const customerId of customerIds) {
            ...
            // Sync campaigns
            const campaignCount = await syncCampaigns(organizationId, platform.id, account.id, customerId, accessToken);
            ...
            // Sync cost metrics
            const metricsResult = await syncCostMetrics(organizationId, platform.id, account.id, customerId, accessToken, startDate, endDate, account.timezone);
 ...
 */
export async function runGoogleAdsSyncJob() {

...
import { calculateAttributionWeight, normalizeWeights, getJourneyConfig, } from "../config/journey-builder";
const prisma = new PrismaClient();
/**
 * Build a journey for a single conversion
 */
export async function buildJourneyForConversion(conversion, config = getJourneyConfig()) {
    const lookbackDate = new Date(conversion.occurredAt.getTime() - config.lookbackDays * 24 * 60 * 60 * 1000);
    ...
    for (const model of config.attributionModels) {
        ...
        const weights = touchpoints.map((tp, index) => calculateAttributionWeight(model, index, touchpoints.length, tp.occurredAt, conversion.occurredAt));
        // Normalize weights
 ...
 */
export async function buildMissingJourneys(organizationId, config = getJourneyConfig()) {
    let processed = 0;
    let created = 0;
    let skipped = 0;
 ...
 */
export async function rebuildJourneys(organizationId, config = getJourneyConfig()) {
    let processed = 0;
    let rebuilt = 0;
    let skipped = 0;
 ...
 */
export async function buildJourneyById(conversionId, config = getJourneyConfig()) {
 ...
 */
export async function getJourneyStats(organizationId) {
    const where = organizationId ? { organizationId } : {};
    const [totalJourneys, convertedJourneys, inProgressJourneys, abandonedJourneys, conversionsWithoutJourneys,] = await Promise.all([
 ...
 */
export async function runJourneyBuilderJob(config = getJourneyConfig()) {

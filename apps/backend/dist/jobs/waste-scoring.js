 ...
 */
async function calculateWasteScore(organizationId, date, platformId, adAccountId, campaignId, adId, config = DEFAULT_CONFIG) {
    ...
    const attributionLinks = await prisma.$queryRaw `
    SELECT 
 ...
 */
export async function calculateWasteScoresForOrganization(organizationId, config = DEFAULT_CONFIG) {
 ...
 */
export async function getWasteDashboardMetrics(organizationId, startDate, endDate, attributionModel = "LAST_TOUCH") {
    const scores = await prisma.wasteScore.findMany({
        where: {
            organizationId,
            date: {
 ...
 */
export async function runWasteScoringJob(config = DEFAULT_CONFIG) {
...
//# sourceMappingURL=waste-scoring.js.map
...
import {
  ...
  type ConversionIngestionResponse,
  type Conversion,
 ...
 */
function valueToMicros(value: number): bigint {
 ...
 */
export async function registerConversionRoutes(app: FastifyInstance) {
  ...
  app.post(
    ...
    async (req: FastifyRequest, reply: FastifyReply) => {
      ...
      (req as any).organizationId = site.organizationId;
      ...
      const results = await Promise.all(
        conversions.map((conversion, index) =>
          processConversion(conversion, site.id, site.organizationId, ip, userAgent)
            .then((result) => ({
              ...result,
              index,
      ...
      const response: ConversionIngestionResponse = {
        success: rejected === 0,
        accepted,
        rejected,
        duplicates,
   ...
   * }
   */
  app.post("/api/conversions/hash", async (req: FastifyRequest, reply: FastifyReply) => {
   ...
   */
  app.get("/api/conversions/health", async (req: FastifyRequest, reply: FastifyReply) => {
    try {

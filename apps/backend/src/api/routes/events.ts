 ...
 */
function valueToMicros(value: number | undefined): bigint | null {
...
export async function registerEventRoutes(app: FastifyInstance) {
    ...
    async (req: FastifyRequest, reply: FastifyReply) => {
      ...
      const results = await Promise.all(
        events.map((event, index) =>
          processEvent(event, site.id, ip, userAgent)
   ...
   */
  app.get("/api/events/health", async (req: FastifyRequest, reply: FastifyReply) => {
    try {

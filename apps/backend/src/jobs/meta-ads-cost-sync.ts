...
export async function runMetaAdsCostSyncJob(params?: {
  ...
  const orgIds = params?.organizationId
    ...
    : (await prisma.integrationConnection.findMany({
      ...
      })).map((x: { organizationId: string }) => x.organizationId);
  ...
  for (const organizationId of orgIds) {
    ...
    for (const conn of connections) {
      if (!conn.accessTokenEnc) continue;
      ...
      if (!adAccount) {
        ...
        const accountData = await client.request<any>(conn.externalAccountId, {
      ...
      for await (const page of client.fetchAllPages<MetaInsight>(`${conn.externalAccountId}/insights`, {

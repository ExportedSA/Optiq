...
export interface TouchpointAttribution {
  ...
  weight: number;
  credit: number; // Actual credit amount (weight * conversion value)
  position: number;
}
...
export function calculateAttribution(
  ...
  const rawWeights = touchpoints.map((tp, index) =>
  ...
  const attributions: TouchpointAttribution[] = touchpoints.map((tp, index) => ({
    touchpointId: tp.id,
    model,
 ...
 */
export function calculateMultiModelAttribution(
  conversionId: string,
  conversionValue: number,
  touchpoints: AttributionTouchpoint[],
  conversionTimestamp: Date,
  ...
  config: AttributionConfig = {},
): Map<AttributionModel, AttributionResult> {
 ...
 */
export async function persistAttributionResults(
  siteId: string,
  conversionId: string,
 ...
 */
export async function getAttributionResults(
  conversionId: string,
  model?: AttributionModel,
): Promise<TouchpointAttribution[]> {
  ...
  return links.map((link) => ({
    ...
    model: link.model as AttributionModel,
    weight: link.weight,
...
export function compareAttributionModels(
  ...
  creditDistribution: { min: number; max: number; avg: number };
}[] {
  const comparison: {
    model: AttributionModel;
    topTouchpoint: string;
  ...
  for (const [model, result] of results.entries()) {
    if (result.attributions.length === 0) continue;

    // Find top touchpoint
    const topAttribution = result.attributions.reduce((max, curr) =>
      curr.credit > max.credit ? curr : max,
    );
 ...
 */
export function validateAttributionResult(result: AttributionResult): {
  valid: boolean;
  errors: string[];
} {
  const errors: string[] = [];
  ...
  if (Math.abs(creditSum - result.conversionValue) > 0.01) {
    errors.push(
      `Credits sum to ${creditSum}, expected ${result.conversionValue}`,
    );
  }
 ...
 */
export function getAttributionStats(result: AttributionResult): {
  model: AttributionModel;
  totalCredit: number;
  ...
  concentrationRatio: number; // Top touchpoint credit / total credit
} {

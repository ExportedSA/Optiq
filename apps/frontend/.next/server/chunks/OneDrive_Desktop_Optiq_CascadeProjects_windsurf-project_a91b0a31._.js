...
`,{errors:s,snapshot:i})}function h(e){return{appEnv:e,isDevelopment:"development"===e,isStaging:"staging"===e,isProduction:"production"===e}}e.s(["AppEnvSchema",0,p,"buildEnvMeta",()=>h,"loadEnv",()=>C],37966),e.s([],1466)},12941,e=>{"use strict";e.i(42960);var t=e.i(91490);e.s(["PLAN_DEFINITIONS",()=>t.PLAN_DEFINITIONS,"calculateOverages",()=>t.calculateOverages])},64137,e=>{"use strict";var t=e.i(60806);e.i(25330);var n=e.i(96685);e.i(1466);var r=e.i(12941);async function a(e){let r=n.appLogger.child({job:"usage-aggregation"}),a=e?.backfillDays??(process.env.USAGE_BACKFILL_DAYS?parseInt(process.env.USAGE_BACKFILL_DAYS,10):1);r.info("Starting usage aggregation",{backfillDays:a,organizationId:e?.organizationId});let s={processedSubscriptions:0,processedDays:0,errors:0},o=new Date;o.setHours(0,0,0,0);let c=await t.prisma.subscription.findMany({where:{status:{in:["ACTIVE","TRIALING"]}},select:{id:!0,organizationId:!0,plan:!0,monthlyEventLimit:!0,currentPeriodStart:!0,currentPeriodEnd:!0}}),l=e?.organizationId?c.filter(t=>t.organizationId===e.organizationId):c;r.info(`Processing ${l.length} subscriptions`);for(let e=a;e>=1;e--){let t=new Date(o);t.setDate(t.getDate()-e);let n=new Date(t);for(let e of(n.setDate(n.getDate()+1),r.debug(`Processing date: ${t.toISOString().split("T")[0]}`),l))try{await i(e,t,n,r),s.processedDays++}catch(t){r.error(`Error aggregating usage for subscription ${e.id}`,t),s.errors++}s.processedSubscriptions=l.length}return r.info("Usage aggregation completed",{...s}),s}async function i(e,n,a,i){let{organizationId:s}=e,o=(await t.prisma.trackingSite.findMany({where:{organizationId:s},select:{id:!0}})).map(e=>e.id),c=0,l=0,d=0;if(o.length>0)for(let e of(await t.prisma.$queryRaw`
      ...
      WHERE "siteId" = ANY(${o})
        ...
        AND "occurredAt" < ${a}
      GROUP BY "eventType"
    `)){let t=Number(e.count);switch(e.eventType){case"PAGE_VIEW":c=t;break;case"CONVERSION":l=t;break;default:d+=t}}let m=c+l+d,u=await t.prisma.adAccount.count({where:{organizationId:s,status:"ACTIVE"}}),g=await t.prisma.campaign.count({where:{organizationId:s,status:"ACTIVE"}});await t.prisma.$executeRaw`
  ...
  `,p=Number(E[0]?.total??0),v=e.plan,C=r.PLAN_DEFINITIONS[v]??r.PLAN_DEFINITIONS.FREE,h={maxWorkspaces:C.limits.maxWorkspaces,maxConnectors:C.limits.maxConnectors,monthlyEventLimit:e.monthlyEventLimit??C.limits.monthlyEventLimit,dataRetentionDays:C.limits.dataRetentionDays,attributionModels:C.limits.attributionModels,alertsEnabled:C.limits.alertsEnabled,ssoEnabled:C.limits.ssoEnabled,prioritySupport:C.limits.prioritySupport},A=(0,r.calculateOverages)({trackedEvents:p,connectedAccounts:u,workspacesUsed:1},h,C.pricing);await t.prisma.$executeRaw`
    ...
    DO UPDATE SET
      ...
      "updatedAt" = NOW()
  `,i.debug(`Aggregated usage for org ${s}: ${m} events yesterday, ${p} period total`)}e.s(["runUsageAggregation",()=>a])}];

//# sourceMappingURL=OneDrive_Desktop_Optiq_CascadeProjects_windsurf-project_a91b0a31._.js.map